...
mindplot.Messages.BUNDLES={};mindplot.TopicEventDispatcher=new Class({Extends:Events,Static:{_instance:null,configure:function(b){this._instance=new mindplot.TopicEventDispatcher(b)
...
i.setDescription(h);
var f=j.getBranches();
f.each(function(b){var a=i.createNode(b.getType(),b.getId());
b.copyTo(a);
i.addBranch(a)
})
...
},appendChild:function(b){throw"Unsupported operation"
...
j.each(function(b){var a=i.getProperty(b);
h.putProperty(b,a)
});
var f=this.getChildren();
var g=h.getMindmap();
f.each(function(b){var a=g.createNode(b.getType(),b.getId());
b.copyTo(a);
h.appendChild(a)
});
return h
...
c.each(function(a){d=d+"=> (";
var b=a.getPropertiesKeys();
b.each(function(h){var g=a.getProperty(h);
d=d+h+":"+g+","
});
d=d+"}"
}.bind(this))
...
return d
},appendChild:function(b){$assert(b&&b.isNodeModel(),"Only NodeModel can be appended to Mindmap object");
...
}});mindplot.ActionDispatcher=new Class({Implements:[Events],initialize:function(b){$assert(b,"commandContext can not be null")
...
},deleteTopic:function(b){this._designer._removeTopic(b)
...
},deleteRelationship:function(b){this._designer._deleteRelationship(b)
...
}});mindplot.DesignerModel=new Class({Implements:[Events],initialize:function(b){this._zoom=b.zoom;
this._topics=[];
this._relationships=[]
...
}});mindplot.Designer=new Class({Extends:Events,initialize:function(e,f){$assert(e,"options must be defined");
...
this._actionDispatcher.addEvent("modelUpdate",function(a){this.fireEvent("modelUpdate",a)
}.bind(this));
...
$(document).addEvent("mousewheel",function(f){var b=c.getContainer().getCoordinates();
var a=f.client.y<b.top||f.client.y>b.bottom||f.client.x<b.left||f.client.x>b.right;
if(!a){if(f.wheel>0){this.zoomIn(1.05)
...
}},_registerMouseEvents:function(){var d=this._workspace;
var f=d.getScreenManager();
f.addEvent("update",function(){var a=this.getModel().getTopics();
a.each(function(b){b.closeEditors()
});
if(this._cleanScreen){this._cleanScreen()
}}.bind(this));
f.addEvent("click",function(a){this.onObjectFocusEvent(null,a)
}.bind(this));
f.addEvent("dblclick",function(b){if(d.isWorkspaceEventsEnabled()){var h=f.getWorkspaceMousePosition(b);
...
},_buildNodeGraph:function(n,k){var m=mindplot.NodeGraph.create(n,{readOnly:k});
this.getModel().addTopic(m);
if(!k){m.addEvent("mousedown",function(a){this.onObjectFocusEvent(m,a)
}.bind(this));
if(m.getType()!=mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE){this._dragManager.add(m)
}}var p=n.isConnected();
if(p){var l=n.getParent();
var q=null;
...
}m.addEvent("ontblur",function(){var a=this.getModel().filterSelectedTopics();
var b=this.getModel().filterSelectedRelationships();
if(a.length==0||b.length==0){this.fireEvent("onblur")
}}.bind(this));
m.addEvent("ontfocus",function(){var a=this.getModel().filterSelectedTopics();
var b=this.getModel().filterSelectedRelationships();
if(a.length==1||b.length==1){this.fireEvent("onfocus")
}}.bind(this));
return m
},onObjectFocusEvent:function(g,j){var h=this.getModel().getTopics();
h.each(function(a){a.closeEditors()
});
var f=this.getModel();
var i=f.getEntities();
i.each(function(a){if(!$defined(j)||(!j.control&&!j.meta)){if(a.isOnFocus()&&a!=g){a.setOnFocus(false)
}}})
},selectAll:function(){var d=this.getModel();
var c=d.getEntities();
c.each(function(a){a.setOnFocus(true)
})
},deselectAll:function(){var b=this.getModel().getEntities();
b.each(function(a){a.setOnFocus(false)
})
...
},loadMap:function(s){$assert(s,"mindmapModel can not be null");
this._mindmap=s;
var i={width:25,height:25};
var q=new mindplot.layout.LayoutManager(s.getCentralTopic().getId(),i);
q.addEvent("change",function(b){var a=b.getId();
...
for(var p=0;
p<n.length;
p++){this._relationshipModelToRelationship(n[p])
...
},_nodeModelToNodeGraph:function(h){$assert(h,"Node model can not be null");
...
f<j.length;
f++){var i=j[f];
if($defined(i)){this._nodeModelToNodeGraph(i)
}}return g
...
return e
},_addRelationship:function(c){var d=this.getMindmap();
d.addRelationship(c);
return this._relationshipModelToRelationship(c)
},_deleteRelationship:function(e){var f=e.getSourceTopic();
...
},_buildRelationshipShape:function(m){var j=this.getModel();
var k=m.getFromNode();
var h=j.findTopicById(k);
var l=m.getToNode();
var n=j.findTopicById(l);
$assert(n,"targetTopic could not be found:"+l+j.getTopics().map(function(a){return a.getId()
}));
var i=new mindplot.Relationship(h,n,m);
i.addEvent("ontblur",function(){var a=this.getModel().filterSelectedTopics();
var b=this.getModel().filterSelectedRelationships();
if(a.length==0||b.length==0){this.fireEvent("onblur")
}}.bind(this));
i.addEvent("ontfocus",function(){var a=this.getModel().filterSelectedTopics();
var b=this.getModel().filterSelectedRelationships();
if(a.length==1||b.length==1){this.fireEvent("onfocus")
}}.bind(this));
j.addRelationship(i);
return i
},_removeTopic:function(f){if(!f.isCentralTopic()){var d=f._parent;
f.disconnect(this._workspace);
while(f.getChildren().length>0){this._removeTopic(f.getChildren()[0])
...
this._clickEvents=[];
this._divContainer.addEvent("click",function(a){a.stopPropagation()
}.bind(this));
this._divContainer.addEvent("dblclick",function(a){a.stopPropagation();
a.preventDefault()
})
},setScale:function(b){$assert(b,"Screen scale can not be null");
this._scale=b
},addEvent:function(d,c){if(d=="click"){this._clickEvents.push(c)
}else{this._divContainer.addEvent(d,c)
}},removeEvent:function(d,c){if(d=="click"){this._clickEvents.remove(c)
}else{this._divContainer.removeEvent(d,c)
}},fireEvent:function(d,c){if(d=="click"){this._clickEvents.each(function(a){a(d,c)
})
}else{this._divContainer.fireEvent(d,c)
...
},getWorkspaceMousePosition:function(h){var e=h.client.x;
var g=h.client.y;
...
},appendChild:function(b){if($defined(b.addToWorkspace)){b.addToWorkspace(this)
}else{this._workspace.appendChild(b)
...
}});mindplot.ShirinkConnector=new Class({initialize:function(d){var f=new web2d.Elipse(mindplot.Topic.prototype.INNER_RECT_ATTRIBUTES);
this._ellipse=f;
f.setFill("rgb(62,118,179)");
f.setSize(mindplot.Topic.CONNECTOR_WIDTH,mindplot.Topic.CONNECTOR_WIDTH);
f.addEvent("click",function(c){var j=d.getModel();
...
f.addEvent("dblclick",function(a){a.stopPropagation()
});
f.addEvent("mouseover",function(a){f.setFill("rgb(153, 0, 255)")
});
f.addEvent("mouseout",function(a){var b=d.getBackgroundColor();
this.setFill(b)
}.bind(this));
f.setCursor("default");
...
},addToWorkspace:function(b){b.appendChild(this._ellipse)
...
}});mindplot.DesignerKeyboard=new Class({Extends:Keyboard,Static:{register:function(b){this._instance=new mindplot.DesignerKeyboard(b);
...
},_registerEvents:function(k){var g=k.getModel();
var l={backspace:function(a){a.preventDefault();
a.stopPropagation();
k.deleteSelectedEntities()
}.bind(this),space:function(){k.shrinkSelectedBranch()
}.bind(this),f2:function(){var a=g.selectedTopic();
if(a){a.showTextEditor()
}}.bind(this),"delete":function(a){k.deleteSelectedEntities();
a.preventDefault();
a.stopPropagation()
}.bind(this),enter:function(){k.createSiblingForSelectedNode()
}.bind(this),insert:function(a){k.createChildForSelectedNode();
a.preventDefault();
a.stopPropagation()
...
}.bind(this),"meta+enter":function(a){a.preventDefault();
a.stopPropagation();
k.createChildForSelectedNode()
}.bind(this),"ctrl+z":function(a){a.preventDefault(a);
a.stopPropagation();
k.undo()
}.bind(this),"meta+z":function(a){a.preventDefault();
a.stopPropagation();
k.undo()
}.bind(this),"ctrl+c":function(a){a.preventDefault(a);
a.stopPropagation();
k.copyToClipboard()
}.bind(this),"meta+c":function(a){a.preventDefault();
a.stopPropagation();
k.copyToClipboard()
}.bind(this),"ctrl+v":function(a){a.preventDefault(a);
a.stopPropagation();
k.pasteClipboard()
}.bind(this),"meta+v":function(a){a.preventDefault();
a.stopPropagation();
k.pasteClipboard()
}.bind(this),"ctrl+z+shift":function(a){a.preventDefault();
a.stopPropagation();
k.redo()
}.bind(this),"meta+z+shift":function(a){a.preventDefault();
a.stopPropagation();
k.redo()
}.bind(this),"ctrl+y":function(a){a.preventDefault();
a.stopPropagation();
k.redo()
}.bind(this),"meta+y":function(a){a.preventDefault();
a.stopPropagation();
k.redo()
}.bind(this),"ctrl+a":function(a){a.preventDefault();
a.stopPropagation();
k.selectAll()
},"ctrl+b":function(a){a.preventDefault();
a.stopPropagation();
k.changeFontWeight()
},"meta+b":function(a){a.preventDefault();
a.stopPropagation();
k.changeFontWeight()
},"ctrl+s":function(a){a.preventDefault();
a.stopPropagation();
$("save").fireEvent("click")
},"meta+s":function(a){a.preventDefault();
a.stopPropagation();
$("save").fireEvent("click")
},"ctrl+i":function(a){a.preventDefault();
a.stopPropagation();
k.changeFontStyle()
},"meta+i":function(a){a.preventDefault();
a.stopPropagation();
k.changeFontStyle()
},"meta+shift+a":function(a){a.preventDefault();
a.stopPropagation();
k.deselectAll()
},"ctrl+shift+a":function(a){a.preventDefault();
a.stopPropagation();
k.deselectAll()
},"meta+a":function(a){a.preventDefault();
a.stopPropagation();
k.selectAll()
},"meta+=":function(a){a.preventDefault();
a.stopPropagation();
k.zoomIn()
},"meta+-":function(a){a.preventDefault();
a.stopPropagation();
k.zoomOut()
},"ctrl+=":function(a){a.preventDefault();
a.stopPropagation();
k.zoomIn()
},"ctrl+-":function(a){a.preventDefault();
a.stopPropagation();
k.zoomOut()
},right:function(b){var c=g.selectedTopic();
if(c){if(c.isCentralTopic()){this._goToSideChild(k,c,"RIGHT")
}else{if(c.getPosition().x<0){this._goToParent(k,c)
}else{if(!c.areChildrenShrunken()){this._goToChild(k,c)
}}}}else{var a=g.getCentralTopic();
this._goToNode(k,a)
}b.preventDefault();
b.stopPropagation()
}.bind(this),left:function(b){var c=g.selectedTopic();
if(c){if(c.isCentralTopic()){this._goToSideChild(k,c,"LEFT")
}else{if(c.getPosition().x>0){this._goToParent(k,c)
}else{if(!c.areChildrenShrunken()){this._goToChild(k,c)
}}}}else{var a=g.getCentralTopic();
this._goToNode(k,a)
...
b.stopPropagation()
}.bind(this),up:function(b){var c=g.selectedTopic();
if(c){if(!c.isCentralTopic()){this._goToBrother(k,c,"UP")
}}else{var a=g.getCentralTopic();
this._goToNode(k,a)
}b.preventDefault();
b.stopPropagation()
}.bind(this),down:function(b){var c=g.selectedTopic();
if(c){if(!c.isCentralTopic()){this._goToBrother(k,c,"DOWN")
}}else{var a=g.getCentralTopic();
this._goToNode(k,a)
}b.preventDefault();
b.stopPropagation()
...
if(!Browser.Platform.mac){i.push("alt")
}$(document).addEvent("keydown",function(c){var d=[];
h.each(function(m){if(c[m]){d.push(m)
}});
if(!j.test(c.key)){d.push(c.key)
}var f=d.join("+");
var b=false;
for(var e in l){if(e==f){b=true;
break
}}if(!b&&!i.contains(f)&&!i.contains(c.key)&&!c.meta&&!c.control){var n=k.getModel().filterSelectedTopics();
if(n.length>0){var a=c.key;
if(h.contains(c.key)){a=""
...
c.setOnFocus(true)
}});mindplot.TopicStyle=new Class({Static:{_getStyles:function(f){$assert(f,"topic can not be null");
var e;
...
this.addEvent("dblclick",function(b){this._getTopicEventDispatcher().show(this);
b.stopPropagation()
}.bind(this))
...
f.setText(g)
}},setText:function(b){if(!b||b.trim().length==0){b=null
...
},_registerDefaultListenersToElement:function(h,f){var g=function(a){if(f.isMouseEventsEnabled()){f.handleMouseOver(a)
}};
h.addEvent("mouseover",g);
var e=function(a){if(f.isMouseEventsEnabled()){f.handleMouseOut(a)
}};
h.addEvent("mouseout",e);
h.addEvent("mousedown",function(b){if(!this.isReadOnly()){var a=true;
if((b.meta&&Browser.Platform.mac)||(b.control&&!Browser.Platform.mac)){a=!this.isOnFocus();
b.stopPropagation();
b.preventDefault()
}f.setOnFocus(a)
...
},setChildrenShrunken:function(i){var f=this.getModel();
f.setChildrenShrunken(i);
var g=this.getShrinkConnector();
if($defined(g)){g.changeRender(i)
}var j=this._flatten2DElements(this);
var h=new mindplot.util.FadeEffect(j,!i);
h.addEvent("complete",function(){if(i){this.setOnFocus(true)
}j.forEach(function(a){if(a.setOnFocus){a.setOnFocus(false)
}})
}.bind(this));
h.start();
...
return b.isVisible()
},_setRelationshipLinesVisibility:function(b){this._relationships.each(function(g){var a=g.getSourceTopic();
...
}},appendChild:function(c){var d=this.getChildren();
d.push(c)
...
}}},updateFreeLayout:function(f){var d=(f.meta&&Browser.Platform.mac)||(f.control&&!Browser.Platform.mac);
...
},addToWorkspace:function(c){if(!this._isInWorkspace){c.appendChild(this._elem2d);
var d=this._getDragPivot();
d.addToWorkspace(c);
this._isInWorkspace=true
...
},add:function(h){var j=this._workspace;
var f=j.getScreenManager();
var i=this;
var g=function(c){if(j.isWorkspaceEventsEnabled()){j.enableWorkspaceEvents(false);
...
j++){if(g[j]==h){i=true;
f=j
}}},_buildMouseMoveListener:function(j,h,i){var f=j.getScreenManager();
var g=function(b){if(!this._isDragInProcess){var d=i._listeners.startdragging;
d(b,h);
j.appendChild(h);
...
return g
},_buildMouseUpListener:function(l,j,i,k){var g=l.getScreenManager();
var h=function(a){$assert(i.isDragTopic,"dragNode must be an DragTopic");
...
window.document.body.style.cursor="default";
if(this._isDragInProcess){var b=k._listeners.enddragging;
b(a,i);
...
},addToWorkspace:function(b){b.appendChild(this._line2d);
...
},addToWorkspace:function(b){b.appendChild(this._focusShape);
...
if(this._endArrow){b.appendChild(this._endArrow)
}this.parent(b);
this._positionArrows();
this.redraw()
...
}},_searchConnectionCandidates:function(n){var j=this._designerModel.getTopics();
var h=n.getDraggedTopic();
var k=n.getSize?n.getSize().width:0;
var l=n.getPosition().x>0?0:k;
var i={x:n.getPosition().x-l,y:n.getPosition().y};
j=j.filter(function(a){var b=h!=a;
b=b&&a!=h;
b=b&&!a.areChildrenShrunken()&&!a.isCollapsed();
b=b&&!h.isChildTopic(a);
return b
});
j=j.filter(function(b){var c=b.getPosition();
...
j=j.sort(function(c,e){var d=c.getPosition();
var a=e.getPosition();
...
f.inject(i);
return g
},_registerEvents:function(f){var d=this._getTextareaElem();
var e=this._getSpanElem();
f.addEvent("keydown",function(a){switch(a.key){case"esc":this.close(false);
break;
case"enter":this.close(true);
break;
default:e.innerHTML=d.value;
var b=d.value.length+1;
d.size=b;
if(e.offsetWidth>(parseInt(f.style.width)-100)){f.style.width=(e.offsetWidth+100)+"px"
}break
}a.stopPropagation()
}.bind(this));
f.addEvent("click",function(a){a.stopPropagation()
});
f.addEvent("dblclick",function(a){a.stopPropagation()
});
f.addEvent("mousedown",function(a){a.stopPropagation()
...
this._setText(i);
var k=function(){var c=this._topic.getTextShape();
c.positionRelativeTo(this._containerElem,{position:{x:"left",y:"top"},edge:{x:"left",y:"top"}});
...
this._timeoutId=-1
},_buildEditor:function(){var d=new Element("div");
d.setStyles({position:"absolute",display:"none",zIndex:"8",overflow:"hidden",border:"0 none"});
var c=new Element("textarea",{tabindex:"-1",value:"",wrap:"off"});
c.setStyles({border:"1px gray dashed",background:"rgba(98, 135, 167, .3)",outline:"0 none",resize:"none",overflow:"hidden"});
c.inject(d);
return d
},_registerEvents:function(d){var c=this._getTextareaElem();
c.addEvent("keydown",function(b){switch(b.key){case"esc":this.close(false);
break;
case"enter":if(b.meta||b.control){var a=c.value;
var l=a.length;
if(c.selectionStart){l=c.selectionStart
}var i=a.substring(0,l);
var j="";
if(l<a.length){j=a.substring(l,a.length)
}c.value=i+"\n"+j;
if(c.setSelectionRange){c.focus();
c.setSelectionRange(l+1,l+1)
}else{if(c.createTextRange){var k=c.createTextRange();
...
c.addEvent("keypress",function(a){a.stopPropagation()
});
c.addEvent("keyup",function(b){var a=this._getTextareaElem().value;
this.fireEvent("input",[b,a]);
this._adjustEditorSize()
}.bind(this));
d.addEvent("click",function(a){a.stopPropagation()
});
d.addEvent("dblclick",function(a){a.stopPropagation()
});
...
var d=f.value.split("\n");
var e=1;
d.each(function(a){if(e<a.length){e=a.length
}});
f.setAttribute("cols",e);
f.setAttribute("rows",d.length);
this._containerElem.setStyles({width:(e+3)+"em",height:f.getSize().height})
}},isVisible:function(){return $defined(this._containerElem)&&this._containerElem.getStyle("display")=="block"
},_updateModel:function(){if(this._topic.getText()!=this._getText()){var f=this._getText();
var e=this._topic.getId();
var d=mindplot.ActionDispatcher.getInstance();
...
this._showEditor(f)
}},_showEditor:function(j){var f=this._topic;
...
g.color=h.getColor();
this._setStyle(g);
var i=function(){var b=f.getTextShape();
b.positionRelativeTo(this._containerElem,{position:{x:"left",y:"top"},edge:{x:"left",y:"top"}});
this._containerElem.setStyle("display","block");
...
},_positionCursor:function(g,h){g.focus();
if(h){if(g.createTextRange){var e=g.createTextRange();
e.select();
e.move("character",g.value.length)
}else{g.setSelectionRange(0,g.value.length)
}}else{if(g.createTextRange){var f=g.createTextRange();
f.move("character",g.value.length)
}else{g.selectionStart=g.value.length
...
i.y=Math.ceil(i.y);
return i
}};mindplot.util.FadeEffect=new Class({Extends:Fx,initialize:function(c,d){this.parent({duration:3000,frames:15,transition:"linear"});
this._isVisible=d;
this._element=c;
this.addEvent("complete",function(){this._element.each(function(a){if(a){a.setVisibility(d)
}})
})
},start:function(){this.parent(this._isVisible?0:1,this._isVisible?1:0)
},set:function(b){this._element.each(function(a){if(a){a.setOpacity(b)
}});
return this
...
}if(this._idsMap.has(j)){j=null
}else{this._idsMap.set(j,ak)
...
},loadFromDom:function(g,h){$assert($defined(h),"mapId can not be null");
var f=this._betaSerializer.loadFromDom(g,h);
f.setVersion(mindplot.persistence.ModelCodeName.PELA);
var e=f.getBranches();
e.each(function(a){this._fixPosition(a)
}.bind(this));
return f
},_fixPosition:function(e){var f=e.getPosition();
var d=f.x>0;
e.getChildren().each(function(a){if(!a.getPosition()){a.setPosition(f.x+(50*d?1:-1),f.y)
}this._fixPosition(a)
}.bind(this))
...
},saveMapXml:function(m,t,k,n,l,p){var r={id:m,xml:t,properties:k};
var o=this;
var q="minor="+!n;
q=q+"&timestamp="+this.timestamp;
q=q+"&session="+this.session;
if(!o.onSave){o.onSave=true;
o.clearTimeout=setTimeout(function(){o.clearTimeout=null;
o.onSave=false
},10000);
var s=new Request({url:this.documentUrl.replace("{id}",m)+"?"+q,method:"put",async:!p,onSuccess:function(b,a){o.timestamp=b;
l.onSuccess()
},onException:function(a,b){l.onError(o._buildError())
},onComplete:function(){if(o.clearTimeout){clearTimeout(o.clearTimeout)
}o.onSave=false
},onFailure:function(e){var a=e.responseText;
var c={severity:"SEVERE",message:$msg("SAVE_COULD_NOT_BE_COMPLETED")};
var d=this.getHeader("Content-Type");
if(d!=null&&d.indexOf("application/json")!=-1){var b=null;
try{b=JSON.decode(a);
b=b.globalSeverity?b:null
}catch(f){}c=o._buildError(b)
}else{if(this.status==405){c={severity:"SEVERE",message:$msg("SESSION_EXPIRED")}
}}l.onError(c);
o.onSave=false
},headers:{"Content-Type":"application/json; charset=utf-8",Accept:"application/json"},emulation:false,urlEncoded:false});
s.put(JSON.encode(r))
}},discardChanges:function(d){var c=new Request({url:this.revertUrl.replace("{id}",d),async:false,method:"post",onSuccess:function(){},onException:function(){},onFailure:function(){},headers:{"Content-Type":"application/json; charset=utf-8",Accept:"application/json"},emulation:false,urlEncoded:false});
c.post()
},unlockMap:function(e){var d=e.getId();
var f=new Request({url:this.lockUrl.replace("{id}",d),async:false,method:"put",onSuccess:function(){},onException:function(){},onFailure:function(){},headers:{"Content-Type":"text/plain"},emulation:false,urlEncoded:false});
f.put("false")
...
},loadMapDom:function(e){var f;
var h=new Request({url:this.documentUrl.replace("{id}",e)+"/xml",method:"get",async:false,headers:{"Content-Type":"text/plain",Accept:"application/xml"},onSuccess:function(a){f=a
}});
h.send();
if(f==null){throw new Error("Map could not be loaded")
}var g=new DOMParser();
...
},loadMapDom:function(e){var f=localStorage.getItem(e+"-xml");
if(f==null||this.forceLoad){var h=new Request({url:this.documentUrl.replace("{id}",e),headers:{"Content-Type":"text/plain",Accept:"application/xml"},method:"get",async:false,onSuccess:function(a){f=a
}});
h.send();
if(f==null){throw new Error("Map could not be loaded")
}}var g=new DOMParser();
...
this._icons.each(function(a){var b=a.getModel();
...
this._removeIcon(d)
},_removeIcon:function(b){$assert(b,"icon can not be null");
this._removeTip.close(0);
this._group.removeChild(b.getImage());
this._icons.erase(b);
this._resize(this._icons.length);
this._icons.each(function(d,a){this._positionIcon(d,a)
}.bind(this))
...
this._fadeElem=b
},show:function(e,f){$assert(f,"icon can not be null");
if(this._activeIcon!=f){if(this._activeIcon){this.close(0)
}var g=f.getPosition();
var h=this._buildWeb2d();
h.addEvent("click",function(){f.remove()
});
h.addEvent("mouseover",function(){this.show(e,f)
}.bind(this));
h.addEvent("mouseout",function(){this.hide()
}.bind(this));
...
},close:function(e){if(this._closeTimeoutId){clearTimeout(this._closeTimeoutId)
}if(this._activeIcon){var d=this._widget;
var f=function(){this._activeIcon=null;
this._fadeElem.removeChild(d);
this._widget=null;
this._closeTimeoutId=null
}.bind(this);
if(!$defined(e)||e==0){f()
}else{this._closeTimeoutId=f.delay(e)
...
f.setSize(50,50);
return f
},decorate:function(c,d){if(!d.__remove){d.addEvent("mouseover",function(){this.show(c,d)
}.bind(this));
d.addEvent("mouseout",function(){this.hide()
}.bind(this));
d.__remove=true
...
if(!this._readOnly){this.addEvent("click",function(b){this._topic.showLinkEditor();
b.stopPropagation()
}.bind(this))
...
if(!this._readOnly){this.addEvent("click",function(b){this._topic.showNoteEditor();
b.stopPropagation()
}.bind(this))
}this._tip=new mindplot.widget.FloatingTip(this.getImage()._peer._native,{content:function(){var e=new Element("div");
e.setStyles({padding:"5px"});
var f=new Element("div",{text:$msg("NOTE")});
f.setStyles({"font-weight":"bold",color:"black","padding-bottom":"5px",width:"100px"});
f.inject(e);
var d=new Element("div",{text:this._linksModel.getText()});
d.setStyles({"white-space":"pre-wrap","word-wrap":"break-word"});
d.inject(e);
return e
...
},addToGroup:function(b){b.appendChild(this.getImage())
...
}});mindplot.ImageIcon=new Class({Extends:mindplot.Icon,initialize:function(g,l,i){$assert(l,"iconModel can not be null");
...
this.parent(j);
if(!i){var k=this.getImage();
k.addEvent("click",function(){var c=l.getIconType();
...
this._type=b;
this._attributes={};
this["is"+b.camelCase()+"Model"]=function(){return true
}
...
}});mindplot.ControlPoint=new Class({initialize:function(){var c=new web2d.Elipse({width:6,height:6,stroke:"1 solid #6589de",fillColor:"gray",visibility:false});
c.setCursor("pointer");
var d=new web2d.Elipse({width:6,height:6,stroke:"1 solid #6589de",fillColor:"gray",visibility:false});
d.setCursor("pointer");
...
this._controlPointsController[0].addEvent("mousedown",function(a){(this._mouseDown.bind(this))(a,mindplot.ControlPoint.FROM)
}.bind(this));
this._controlPointsController[0].addEvent("click",function(a){(this._mouseClick.bind(this))(a)
}.bind(this));
this._controlPointsController[0].addEvent("dblclick",function(a){(this._mouseClick.bind(this))(a)
}.bind(this));
this._controlPointsController[1].addEvent("mousedown",function(a){(this._mouseDown.bind(this))(a,mindplot.ControlPoint.TO)
}.bind(this));
this._controlPointsController[1].addEvent("click",function(a){(this._mouseClick.bind(this))(a)
}.bind(this));
this._controlPointsController[1].addEvent("dblclick",function(a){(this._mouseClick.bind(this))(a)
}.bind(this))
},setSide:function(b){this._side=b
...
},_removeLine:function(){},_mouseDown:function(c,d){if(!this._isBinded){this._isBinded=true;
this._mouseMoveFunction=function(a){(this._mouseMoveEvent.bind(this))(a,d)
}.bind(this);
this._workspace.getScreenManager().addEvent("mousemove",this._mouseMoveFunction);
this._mouseUpFunction=function(a){(this._mouseUp.bind(this))(a,d)
}.bind(this);
this._workspace.getScreenManager().addEvent("mouseup",this._mouseUpFunction)
}c.preventDefault();
c.stop();
return false
},_mouseMoveEvent:function(k,h){var g=this._workspace.getScreenManager();
var i=g.getWorkspaceMousePosition(k);
var l=null;
if(h==0){var j=mindplot.util.Shape.calculateRelationShipPointCoordinates(this._line.getSourceTopic(),i);
this._line.setFrom(j.x,j.y);
this._line.setSrcControlPoint(new core.Point(i.x-j.x,i.y-j.y))
}else{var j=mindplot.util.Shape.calculateRelationShipPointCoordinates(this._line.getTargetTopic(),i);
...
var j=n.getTopics();
j.each(function(a){a.addEvent("ontfocus",this._onTopicClick)
}.bind(this))
}},dispose:function(){var e=this._workspace;
if(this._isActive()){e.removeEvent("mousemove",this._mouseMoveEvent);
e.removeEvent("click",this._onClickEvent);
var d=this._designer.getModel();
var f=d.getTopics();
f.each(function(a){a.removeEvent("ontfocus",this._onTopicClick)
}.bind(this));
...
},_connectOnFocus:function(g){var f=this._sourceTopic;
var e=this._designer.getMindmap();
if(g.getId()!=f.getId()){var h=e.createRelationship(g.getId(),f.getId());
...
this._commandFunc=f;
this._oldValues=[]
},execute:function(e){if(!this.applied){var f=null;
try{f=e.findTopics(this._topicsId)
}catch(d){if(this._commandFunc.commandType!="changeTextToTopic"){throw d
}}if(f!=null){f.each(function(a){var b=this._commandFunc(a,this._value);
this._oldValues.push(b)
}.bind(this))
}this.applied=true
}else{throw"Command can not be applied two times in a row."
}},undoExecute:function(d){if(this.applied){var c=d.findTopics(this._topicsId);
c.each(function(a,b){this._commandFunc(a,this._oldValues[b])
}.bind(this));
this.applied=false;
this._oldValues=[]
...
if(e.length>0){e.each(function(a){this._deletedRelModel.push(a.getModel().clone());
d.deleteRelationship(a)
},this)
}},undoExecute:function(f){this._deletedTopicModels.each(function(a){f.createTopic(a)
},this);
this._deletedTopicModels.each(function(c,i){var a=f.findTopics(c.getId());
var b=this._parentTopicIds[i];
if(b){var j=f.findTopics(b);
f.connect(a[0],j[0])
}},this);
this._deletedRelModel.each(function(a){f.addRelationship(a)
}.bind(this));
this._deletedTopicModels.each(function(b){var a=f.findTopics(b.getId());
...
var f=[];
h.each(function(c){var b=c.getParent();
var a=false;
...
this._models=c;
this._parentsIds=d
},execute:function(b){this._models.each(function(j,k){var l=b.createTopic(j);
if(this._parentsIds){var h=this._parentsIds[k];
if($defined(h)){var a=b.findTopics(h)[0];
b.connect(l,a)
}}else{b.addTopic(l)
}var i=b._designer;
...
this._models.each(function(a){d.push(a.clone())
});
this._models.each(function(b){var a=b.getId();
...
e._refreshShape()
}}});mindplot.widget.ModalDialogNotifier=new Class({Extends:MooDialog,initialize:function(){this.parent({closeButton:false,destroyOnClose:false,autoOpen:true,useEscKey:false,closeOnOverlayClick:false,title:"",onInitialize:function(b){b.setStyle("opacity",0);
this.wrapper.setStyle("display","none");
this.fx=new Fx.Morph(b,{duration:100,transition:Fx.Transitions.Bounce.easeOut})
},onBeforeOpen:function(){var b=this._buildPanel();
this.setContent(b);
...
this.message=null
},show:function(d,c){$assert(d,"message can not be null");
this._messsage=d;
this.options.title=$defined(c)?c:"Outch!!. An unexpected error has occurred";
this.open()
},destroy:function(){this.parent();
this.overlay.destroy()
},_buildPanel:function(){var e=new Element("div");
e.setStyles({"text-align":"center",width:"400px"});
var f=new Element("p",{text:this._messsage});
f.inject(e);
var d=new Element("img",{src:"images/alert-sign.png"});
d.inject(e);
return e
}});
var dialogNotifier=new mindplot.widget.ModalDialogNotifier();
$notifyModal=dialogNotifier.show.bind(dialogNotifier);mindplot.widget.ToolbarNotifier=new Class({initialize:function(){var b=$("headerNotifier");
if(b){this._effect=new Fx.Elements(b,{onComplete:function(){b.setStyle("display","none")
}.bind(this),link:"cancel",duration:8000,transition:Fx.Transitions.Expo.easeInOut})
}},logError:function(b){this.logMessage(b,mindplot.widget.ToolbarNotifier.MsgKind.ERROR)
},hide:function(){},logMessage:function(f,d){$assert(f,"msg can not be null");
var e=$("headerNotifier");
if(e){e.set("text",f);
e.setStyle("display","block");
e.position({relativeTo:$("header"),position:"upperCenter",edge:"centerTop"});
if(!$defined(d)||d){this._effect.start({0:{opacity:[1,0]}})
...
$notify=toolbarNotifier.logMessage.bind(toolbarNotifier);mindplot.widget.ToolbarItem=new Class({Implements:[Events],initialize:function(f,d,e){$assert(f,"buttonId can not be null");
...
if(this._enable){b.removeEvent("click",this._fn);
b.removeClass("buttonOn");
b.addClass("buttonOff");
this._enable=false
}},enable:function(){var b=this.getButtonElem();
if(!this._enable){b.addEvent("click",this._fn);
b.removeClass("buttonOff");
b.addClass("buttonOn");
this._enable=true
}},getTip:function(){return this._tip
}.protect()});mindplot.widget.ToolbarPaneItem=new Class({Extends:mindplot.widget.ToolbarItem,initialize:function(f,e){$assert(f,"buttonId can not be null");
$assert(e,"model can not be null");
this._model=e;
var d=function(){if(this.isVisible()){this.hide()
}else{this.show()
}}.bind(this);
this.parent(f,d,{topicAction:true,relAction:false});
...
var f=this;
this._tip=new mindplot.widget.FloatingTip(d,{html:true,position:"bottom",arrowOffset:5,center:true,arrowSize:7,showDelay:0,hideDelay:0,content:function(){return f._updateSelectedItem()
...
}.protect()});mindplot.widget.NoteEditor=new Class({Extends:MooDialog,initialize:function(c){$assert(c,"model can not be null");
var d=this._buildPanel(c);
this.parent({closeButton:true,destroyOnClose:true,title:$msg("NOTE"),onInitialize:function(a){a.setStyle("opacity",0);
this.fx=new Fx.Morph(a,{duration:600,transition:Fx.Transitions.Bounce.easeOut})
...
this.setContent(d)
},_buildPanel:function(o){var j=new Element("div");
var m=new Element("form",{action:"none",id:"noteFormId"});
var l=new Element("textarea",{placeholder:$msg("WRITE_YOUR_TEXT_HERE"),required:true,autofocus:"autofocus"});
if(o.getValue()!=null){l.value=o.getValue()
}l.setStyles({width:"100%",height:80,resize:"none"});
l.inject(m);
m.addEvent("submit",function(a){a.preventDefault();
a.stopPropagation();
if(l.value){o.setValue(l.value)
}this.close()
}.bind(this));
var p=new Element("div").setStyles({paddingTop:5,textAlign:"right"});
var k=new Element("input",{type:"submit",value:$msg("ACCEPT"),"class":"btn-primary"});
k.addClass("button");
k.inject(p);
if($defined(o.getValue())){var i=new Element("input",{type:"button",value:$msg("REMOVE"),"class":"btn-primary"});
i.setStyle("margin","5px");
i.addClass("button");
i.inject(p);
i.addEvent("click",function(){o.setValue(null);
this.close()
}.bind(this));
p.inject(m)
}var n=new Element("input",{type:"button",value:$msg("CANCEL"),"class":"btn-secondary"});
n.setStyle("margin","5px");
n.addClass("button");
n.inject(p);
n.addEvent("click",function(){this.close()
}.bind(this));
p.inject(m);
j.addEvent("keydown",function(a){a.stopPropagation()
});
m.inject(j);
return j
},show:function(){this.open()
}});mindplot.widget.LinkEditor=new Class({Extends:MooDialog,initialize:function(c){$assert(c,"model can not be null");
var d=this._buildPanel(c);
this.parent({closeButton:true,destroyOnClose:true,title:$msg("LINK"),onInitialize:function(a){a.setStyle("opacity",0);
this.fx=new Fx.Morph(a,{duration:600,transition:Fx.Transitions.Bounce.easeOut})
},onBeforeOpen:function(){this.overlay=new Overlay(this.options.inject,{duration:this.options.duration});
if(this.options.closeOnOverlayClick){this.overlay.addEvent("click",this.close.bind(this))
}this.overlay.open();
...
this.setContent(d)
},_buildPanel:function(p){var k=new Element("div");
k.setStyle("padding-top","15px");
var s=new Element("form",{action:"none",id:"linkFormId"});
var m=new Element("select");
m.setStyles({margin:"5px"});
new Element("option",{text:"URL"}).inject(m);
m.inject(s);
var n=new Element("input",{placeholder:"http://www.example.com/",type:Browser.ie?"text":"url",required:true,autofocus:"autofocus"});
if(p.getValue()!=null){n.value=p.getValue()
}n.setStyles({width:"55%",margin:"0px 10px"});
n.inject(s);
var t=new Element("input",{type:"button",value:$msg("OPEN_LINK")});
t.inject(s);
t.addEvent("click",function(){window.open(n.value,"_blank","status=1,width=700,height=450,resizable=1")
});
s.addEvent("submit",function(a){a.stopPropagation();
a.preventDefault();
if(n.value!=null&&n.value.trim()!=""){p.setValue(n.value)
}this.close()
}.bind(this));
var l=new Element("div").setStyles({paddingTop:5,textAlign:"center"});
var o=new Element("input",{type:"submit",value:$msg("ACCEPT"),"class":"btn-primary"});
o.addClass("button");
o.inject(l);
if($defined(p.getValue())){var q=new Element("input",{type:"button",value:$msg("REMOVE"),"class":"btn-primary"});
q.setStyle("margin","5px");
q.addClass("button");
q.inject(l);
q.addEvent("click",function(a){p.setValue(null);
a.stopPropagation();
this.close()
}.bind(this));
l.inject(s)
}var r=new Element("input",{type:"button",value:$msg("CANCEL"),"class":"btn-secondary"});
r.setStyle("margin","5px");
r.addClass("button");
r.inject(l);
r.addEvent("click",function(){this.close()
}.bind(this));
l.inject(s);
k.addEvent("keydown",function(a){a.stopPropagation()
});
s.inject(k);
return k
},show:function(){this.open()
...
this.boundShow=function(){this.show(c)
}.bind(this);
this.boundHide=function(){this.hide(c)
}.bind(this);
if(!["top","right","bottom","left","inside"].contains(this.options.position)){this.options.position="top"
}this.attach(c)
},attach:function(b){if(b.retrieve("hasEvents")!==null){return
}b.addEvent(this.options.showOn,this.boundShow);
b.addEvent(this.options.hideOn,this.boundHide);
b.store("hasEvents",true)
},show:function(d){var e=$(d).retrieve("floatingtip");
if(e){if(e.getStyle("opacity")==1){clearTimeout(e.retrieve("timeout"));
return this
}}var f=this._create(d);
if(f==null){return this
}d.store("floatingtip",f);
this._animate(f,"in");
if(this.options.preventHideOnOver){f.addEvent(this.options.showOn,this.boundShow);
f.addEvent(this.options.hideOn,this.boundHide)
}this.fireEvent("show",[f,d]);
return this
},hide:function(d){var f=d.retrieve("floatingtip");
if(!f){if(this.options.position=="inside"){try{d=d.getParent().getParent();
f=d.retrieve("floatingtip")
}catch(e){}if(!f){return this
}}else{return this
}}this._animate(f,"out");
this.fireEvent("hide",[f,d]);
return this
},_create:function(u){var x=this.options;
var y=x.content;
var o=x.position;
if(y=="title"){y="floatingtitle";
if(!u.get("floatingtitle")){u.setProperty("floatingtitle",u.get("title"))
}u.set("title","")
}var v=(typeof(y)=="string"?u.get(y):y(u));
var w=new Element("div").addClass(x.className).setStyle("margin",0);
var n=new Element("div").addClass(x.className+"-wrapper").setStyles({margin:0,padding:0,"z-index":w.getStyle("z-index")}).adopt(w);
if(v){if(x.html){v.inject(w)
}else{w.set("text",v)
}}else{return null
}var t=document.id(document.body);
n.setStyles({position:"absolute",opacity:0,top:0,left:0}).inject(t);
if(x.balloon&&!Browser.ie6){var z=new Element("div").addClass(x.className+"-triangle").setStyles({margin:0,padding:0});
var s={"border-color":w.getStyle("background-color"),"border-width":x.arrowSize,"border-style":"solid",width:0,height:0};
switch(o){case"inside":case"top":s["border-bottom-width"]=0;
break;
case"right":s["border-left-width"]=0;
s["float"]="left";
w.setStyle("margin-left",x.arrowSize);
break;
case"bottom":s["border-top-width"]=0;
break;
case"left":s["border-right-width"]=0;
if(Browser.ie7){s.position="absolute";
s.right=0
}else{s["float"]="right"
}w.setStyle("margin-right",x.arrowSize);
break
}switch(o){case"inside":case"top":case"bottom":s["border-left-color"]=s["border-right-color"]="transparent";
s["margin-left"]=x.center?n.getSize().x/2-x.arrowSize:x.arrowOffset;
break;
case"left":case"right":s["border-top-color"]=s["border-bottom-color"]="transparent";
...
var q={x:p.left+x.offset.x,y:p.top+x.offset.y};
if(o=="inside"){n.setStyles({width:n.getStyle("width"),height:n.getStyle("height")});
u.setStyle("position","relative").adopt(n);
q={x:x.offset.x,y:x.offset.y}
}else{switch(o){case"top":q.y-=r.y+x.distance;
break;
case"right":q.x+=p.width+x.distance;
break;
case"bottom":q.y+=p.height+x.distance;
break;
case"left":q.x-=r.x+x.distance;
break
}}if(x.center){switch(o){case"top":case"bottom":q.x+=(p.width/2-r.x/2);
break;
case"left":case"right":q.y+=(p.height/2-r.y/2);
break;
case"inside":q.x+=(p.width/2-r.x/2);
q.y+=(p.height/2-r.y/2);
break
}}n.set("morph",x.fx).store("position",q);
n.setStyles({top:q.y,left:q.x});
return n
},_animate:function(d,c){clearTimeout(d.retrieve("timeout"));
d.store("timeout",(function(i){var b=this.options,h=(c=="in");
var j={opacity:h?1:0};
if((b.motionOnShow&&h)||(b.motionOnHide&&!h)){var a=i.retrieve("position");
if(!a){return
}switch(b.position){case"inside":case"top":j.top=h?[a.y-b.motion,a.y]:a.y-b.motion;
break;
case"right":j.left=h?[a.x+b.motion,a.x]:a.x+b.motion;
break;
case"bottom":j.top=h?[a.y+b.motion,a.y]:a.y+b.motion;
break;
case"left":j.left=h?[a.x-b.motion,a.x]:a.x-b.motion;
break
}}i.morph(j);
if(!h){i.get("morph").chain(function(){this.dispose()
}.bind(i))
}}).delay((c=="in")?this.options.showDelay:this.options.hideDelay,this,d));
return this
}});mindplot.widget.LinkIconTooltip=new Class({Extends:mindplot.widget.FloatingTip,initialize:function(b){$assert(b,"linkIcon can not be null");
this.parent(b.getImage()._peer._native,{content:this._buildContent.pass(b,this),html:true,position:"bottom",arrowOffset:10,center:true,arrowSize:15,offset:{x:10,y:20},className:"linkTip"})
},_buildContent:function(h){var i=new Element("div");
i.setStyles({padding:"5px",width:"100%"});
var j=new Element("div",{text:$msg("LINK")});
j.setStyles({"font-weight":"bold",color:"black","padding-bottom":"5px",width:"100px"});
j.inject(i);
var k=new Element("div",{text:"URL: "+h.getModel().getUrl()});
k.setStyles({"white-space":"pre-wrap","word-wrap":"break-word"});
k.inject(i);
var n=new Element("div");
n.setStyles({width:"100%",textAlign:"right","padding-bottom":"5px","padding-top":"5px"});
var m=new Element("img",{src:"https://api.thumbalizr.com/?url="+h.getModel().getUrl()+"&width=250",img:h.getModel().getUrl(),alt:h.getModel().getUrl()});
m.setStyles({padding:"5px"});
var l=new Element("a",{href:h.getModel().getUrl(),alt:"Open in new window ...",target:"_blank"});
m.inject(l);
l.inject(n);
n.inject(i);
return i
...
var f=g.getChildren();
var j=g.id+"Tip";
var h=new Element("div",{id:j});
f[0].inject(h);
h.inject(g);
this.parent(h,{content:this._buildContent.pass(g,this),html:true,position:"bottom",arrowOffset:10,center:true,arrowSize:3,offset:{x:0,y:-2},className:"keyboardShortcutTip",preventHideOnOver:false,motionOnShow:false,motionOnHide:false,fx:{duration:"100"}});
h.addEvent("click",function(a){h.fireEvent("mouseleave",a)
})
},_buildContent:function(){var d=new Element("div");
d.setStyles({padding:"3px 0px",width:"100%"});
var c=new Element("div",{text:this._text});
c.setStyles({width:"100%",textAlign:"center","font-weight":"bold"});
c.inject(d);
return d
...
},_load:function(){if(!mindplot.widget.ColorPalettePanel._panelContent){Asset.css(this._baseUrl+"/mindmap/public/vendor/wisemapping/css/widget/colorPalette.css",{id:"colorPaletteStyle",title:"colorPalette"});
var d;
var c=new Request({url:this._baseUrl+"/mindmap/public/vendor/wisemapping/css/widget/colorPalette.html",method:"get",async:false,onRequest:function(){},onSuccess:function(a){d=a
},onFailure:function(){d="<div>Sorry, your request failed :(</div>"
}});
c.send();
mindplot.widget.ColorPalettePanel._panelContent=d
}return mindplot.widget.ColorPalettePanel._panelContent
},buildPanel:function(){var d=new Element("div",{"class":"toolbarPanel",id:this._buttonId+"colorPalette"});
d.innerHTML=this._load();
var f=d.getElements("div[class=palette-colorswatch]");
var e=this.getModel();
f.each(function(a){a.addEvent("click",function(){var b=a.getStyle("background-color");
e.setValue(b);
this.hide()
}.bind(this))
}.bind(this));
return d
},_updateSelectedItem:function(){var f=this.getPanelElem();
var i=f.getElements("td[class='palette-cell palette-cell-selected']");
i.each(function(a){a.className="palette-cell"
});
var h=f.getElements("div[class=palette-colorswatch]");
var j=this.getModel();
var g=j.getValue();
h.each(function(a){var b=a.getStyle("background-color");
if(g!=null&&g[0]=="r"){g=g.rgbToHex()
}if(g!=null&&g.toUpperCase()==b.toUpperCase()){a.parentNode.className="palette-cell palette-cell-selected"
}});
return f
...
this._initPanel()
},_initPanel:function(){var b=this.getPanelElem().getElements("div");
b.each(function(a){a.addEvent("click",function(f){f.stopPropagation();
this.hide();
...
d.each(function(a){var b=$defined(a.getAttribute("model"))?a.getAttribute("model"):a.id;
$assert(b,"elemValue can not be null");
if(b==f){a.className="toolbarPanelLinkSelectedLink"
}else{a.className="toolbarPanelLink"
}});
return e
...
},buildPanel:function(){var b=new Element("div",{"class":"toolbarPanel",id:"fontFamilyPanel"});
...
},buildPanel:function(){var b=new Element("div",{"class":"toolbarPanel",id:"fontSizePanel"});
...
},buildPanel:function(){var n=new Element("div",{"class":"toolbarPanel",id:"IconsPanel"});
n.setStyles({width:253,height:230,padding:5});
n.addEvent("click",function(a){a.stopPropagation()
});
var o=0;
for(var r=0;
...
s<i.length;
s=s+1){var m;
if((o%12)==0){m=new Element("div").inject(n)
}var j=i[s];
var q=new Element("img",{id:j,src:mindplot.ImageIcon.prototype._getImageUrl(j)});
q.setStyles({width:16,height:16,padding:"0px 2px",cursor:"pointer"}).inject(m);
var t=this;
var p=this.getModel();
q.addEvent("click",function(a){p.setValue(this.id);
t.hide()
}.bind(q));
o=o+1
}}return n
}});mindplot.widget.IMenu=new Class({initialize:function(f,e,d){$assert(f,"designer can not be null");
...
},clear:function(){this._toolbarElems.each(function(b){b.hide()
})
...
o.save(j,n,m,{onSuccess:function(){if(m){k.setStyle("cursor","pointer");
$notify($msg("SAVE_COMPLETE"))
}l.setRequireChange(false)
},onError:function(a){if(m){k.setStyle("cursor","pointer");
...
}});mindplot.widget.Menu=new Class({Extends:mindplot.widget.IMenu,initialize:function(N,O,ag,aa,am){this.parent(N,O,ag);
am=!$defined(am)?"":am;
var an=am+"css/widget";
$(this._containerId).addEvent("click",function(a){a.stopPropagation();
return false
});
$(this._containerId).addEvent("dblclick",function(a){a.stopPropagation();
return false
});
var ab=N.getModel();
var ae=$("fontFamily");
if(ae){var ai={getValue:function(){var b=ab.filterSelectedTopics();
var c=null;
for(var a=0;
a<b.length;
a++){var d=b[a].getFontFamily();
if(c!=null&&c!=d){c=null;
break
}c=d
}return c
},setValue:function(a){N.changeFontFamily(a)
}};
...
if(af){var ad={getValue:function(){var c=ab.filterSelectedTopics();
var d=null;
for(var b=0;
b<c.length;
b++){var a=c[b].getFontSize();
if(d!=null&&d!=a){d=null;
break
}d=a
}return d
},setValue:function(a){N.changeFontSize(a)
}};
this._toolbarElems.push(new mindplot.widget.FontSizePanel("fontSize",ad));
this._registerTooltip("fontSize",$msg("FONT_SIZE"))
}var S=$("topicShape");
if(S){var ac={getValue:function(){var c=ab.filterSelectedTopics();
var d=null;
for(var b=0;
b<c.length;
b++){var a=c[b].getShapeType();
if(d!=null&&d!=a){d=null;
break
}d=a
}return d
},setValue:function(a){N.changeTopicShape(a)
}};
...
if(W){var ak={getValue:function(){return null
},setValue:function(a){N.addIconType(a)
}};
this._toolbarElems.push(new mindplot.widget.IconPanel("topicIcon",ak));
this._registerTooltip("topicIcon",$msg("TOPIC_ICON"))
}var U=$("topicColor");
if(U){var Y={getValue:function(){var b=ab.filterSelectedTopics();
var d=null;
for(var a=0;
a<b.length;
a++){var c=b[a].getBackgroundColor();
if(d!=null&&d!=c){d=null;
break
}d=c
}return d
},setValue:function(a){N.changeBackgroundColor(a)
}};
...
if(X){var al={getValue:function(){var b=ab.filterSelectedTopics();
var d=null;
for(var a=0;
a<b.length;
a++){var c=b[a].getBorderColor();
if(d!=null&&d!=c){d=null;
break
}d=c
}return d
},setValue:function(a){N.changeBorderColor(a)
}};
...
if(I){var M={getValue:function(){var d=null;
var b=ab.filterSelectedTopics();
for(var a=0;
a<b.length;
a++){var c=b[a].getFontColor();
if(d!=null&&d!=c){d=null;
break
}d=c
}return d
},setValue:function(a){N.changeFontColor(a)
}};
...
}this._addButton("export",false,false,function(){var a=new MooDialog.Request("c/iframeWrapper.htm?url=c/maps/"+ag+"/exportf",null,{"class":"modalDialog exportModalDialog",closeButton:true,destroyOnClose:true,title:$msg("EXPORT")});
a.setRequestOptions({onRequest:function(){a.setContent($msg("LOADING"))
}});
MooDialog.Request.active=a
});
this._registerTooltip("export",$msg("EXPORT"));
this._addButton("print",false,false,function(){this.save(V,N,false);
...
this._addButton("zoomIn",false,false,function(){N.zoomIn()
});
this._registerTooltip("zoomIn",$msg("ZOOM_IN"));
this._addButton("zoomOut",false,false,function(){N.zoomOut()
});
this._registerTooltip("zoomOut",$msg("ZOOM_OUT"));
var ao=this._addButton("undoEdition",false,false,function(){N.undo()
});
if(ao){ao.disable()
}this._registerTooltip("undoEdition",$msg("UNDO"),"meta+Z");
var Q=this._addButton("redoEdition",false,false,function(){N.redo()
});
if(Q){Q.disable()
}this._registerTooltip("redoEdition",$msg("REDO"),"meta+shift+Z");
if(Q&&ao){N.addEvent("modelUpdate",function(a){if(a.undoSteps>0){ao.enable()
}else{ao.disable()
}if(a.redoSteps>0){Q.enable()
}else{Q.disable()
}}.bind(this))
}this._addButton("addTopic",true,false,function(){N.createSiblingForSelectedNode()
});
this._registerTooltip("addTopic",$msg("ADD_TOPIC"),"Enter");
this._addButton("deleteTopic",true,true,function(){N.deleteSelectedEntities()
});
this._registerTooltip("deleteTopic",$msg("TOPIC_DELETE"),"Delete");
this._addButton("topicLink",true,false,function(){N.addLink()
});
this._registerTooltip("topicLink",$msg("TOPIC_LINK"));
this._addButton("topicRelation",true,false,function(a){N.showRelPivot(a)
});
this._registerTooltip("topicRelation",$msg("TOPIC_RELATIONSHIP"));
this._addButton("topicNote",true,false,function(){N.addNote()
});
this._registerTooltip("topicNote",$msg("TOPIC_NOTE"));
this._addButton("fontBold",true,false,function(){N.changeFontWeight()
});
this._registerTooltip("fontBold",$msg("FONT_BOLD"),"meta+B");
this._addButton("fontItalic",true,false,function(){N.changeFontStyle()
});
this._registerTooltip("fontItalic",$msg("FONT_ITALIC"),"meta+I");
var V=$("save");
if(V){this._addButton("save",false,false,function(){this.save(V,N,true)
}.bind(this));
this._registerTooltip("save",$msg("SAVE"),"meta+S");
if(!aa){Element.NativeEvents.unload=1;
$(window).addEvent("unload",function(){if(this.isSaveRequired()){this.save(V,N,false,true)
}this.unlockMap(N)
}.bind(this));
(function(){if(this.isSaveRequired()){this.save(V,N,false)
}}.bind(this)).periodical(30000)
}}var Z=$("discard");
if(Z){this._addButton("discard",false,false,function(){this.discardChanges(N)
}.bind(this));
this._registerTooltip("discard",$msg("DISCARD_CHANGES"))
}var R=$("shareIt");
if(R){this._addButton("shareIt",false,false,function(){var a=new MooDialog.Request("c/iframeWrapper?url=c/maps/"+ag+"/sharef",null,{"class":"modalDialog shareModalDialog",closeButton:true,destroyOnClose:true,title:$msg("COLLABORATE")});
a.setRequestOptions({onRequest:function(){a.setContent($msg("LOADING"))
}});
MooDialog.Request.active=a
});
...
if(ap){this._addButton("publishIt",false,false,function(){var a=new MooDialog.Request("c/iframeWrapper?url=c/maps/"+ag+"/publishf",null,{"class":"modalDialog publishModalDialog",closeButton:true,destroyOnClose:true,title:$msg("PUBLISH")});
a.setRequestOptions({onRequest:function(){a.setContent($msg("LOADING"))
}});
MooDialog.Request.active=a
});
...
if(J){this._addButton("history",false,false,function(){var a=new MooDialog.Request("c/iframeWrapper?url=c/maps/"+ag+"/historyf",null,{"class":"modalDialog historyModalDialog",closeButton:true,destroyOnClose:true,title:$msg("HISTORY")});
a.setRequestOptions({onRequest:function(){a.setContent($msg("LOADING"))
}})
});
this._registerTooltip("history",$msg("HISTORY"))
}this._registerEvents(N);
var ah=$("keyboardShortcuts");
if(ah){ah.addEvent("click",function(b){var a=new MooDialog.Request("c/keyboard",null,{"class":"modalDialog keyboardModalDialog",closeButton:true,destroyOnClose:true,title:$msg("SHORTCUTS")});
a.setRequestOptions({onRequest:function(){a.setContent($msg("LOADING"))
}});
MooDialog.Request.active=a;
b.preventDefault()
...
K.addEvent("click",function(a){window.open("https://www.youtube.com/tv?vq=medium#/watch?v=rKxZwNKs9cE","_blank","toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=yes, copyhistory=no, width="+L+", height="+P+", top="+T+", left="+aj);
a.preventDefault()
})
}},_registerEvents:function(b){this._toolbarElems.each(function(a){a.addEvent("show",function(){this.clear()
}.bind(this))
}.bind(this));
b.addEvent("onblur",function(){var d=b.getModel().filterSelectedTopics();
var a=b.getModel().filterSelectedRelationships();
this._toolbarElems.each(function(c){var g=c.isTopicAction();
var h=c.isRelAction();
if(g||h){if((g&&d.length!=0)||(h&&a.length!=0)){c.enable()
}else{c.disable()
}}})
}.bind(this));
b.addEvent("onfocus",function(){var d=b.getModel().filterSelectedTopics();
var a=b.getModel().filterSelectedRelationships();
this._toolbarElems.each(function(c){var g=c.isTopicAction();
var h=c.isRelAction();
if(g||h){if(g&&d.length>0){c.enable()
}if(h&&a.length>0){c.enable()
}}})
}.bind(this))
},_addButton:function(i,l,g,j){var h=null;
if($(i)){var k=new mindplot.widget.ToolbarItem(i,function(a){j(a);
this.clear()
}.bind(this),{topicAction:l,relAction:g});
...
}});mindplot.layout.LayoutManager=new Class({Extends:Events,initialize:function(g,e){$assert($defined(g),"rootNodeId can not be null");
...
if(!$(b)||b){this._flushEvents()
}return this
},_flushEvents:function(){this._events.each(function(b){this.fireEvent("change",b)
},this);
this._events=[]
},_collectChanges:function(b){if(!b){b=this._treeSet.getTreeRoots()
}b.each(function(f){if(f.hasOrderChanged()||f.hasPositionChanged()){var e=f.getId();
...
}if(JSON.encode(f.value)!=JSON.encode(d)){f.oldValue=f.value;
f.value=d;
f.hasChanged=true
...
t++){var A=z[t];
this._plot(y,A)
}},updateBranchPosition:function(j,h){var g=j.getPosition();
j.setPosition(h);
var k=g.x-h.x;
var i=g.y-h.y;
var l=this.getChildren(j);
l.each(function(a){this.shiftBranchPosition(a,k,i)
}.bind(this))
},shiftBranchPosition:function(i,j,h){var g=i.getPosition();
i.setPosition({x:g.x+j,y:g.y+h});
var f=this.getChildren(i);
f.each(function(a){this.shiftBranchPosition(a,j,h)
}.bind(this))
...
l.each(function(a){m+=this._computeChildrenHeight(j,a,n)
},this);
i=Math.max(h,m)
}if(n){n[k.getId()]=i
}return i
...
K.each(function(a,c){var b=a.getPosition();
...
f.each(function(a,b){if(a.getOrder()>h.getOrder()){a.setOrder(a.getOrder()-2)
}});
...
var x=0;
var B=0;
s.each(function(a){if(a.order%2==0){x+=a.height
}else{B+=a.height
}});
var w=x/2;
var q=B/2;
...
mindplot.layout.SymmetricSorter.INTERNODE_HORIZONTAL_PADDING=30;mindplot.layout.GridSorter=new Class({Extends:mindplot.layout.AbstractBasicSorter,computeOffsets:function(r,x){$assert(r,"treeSet can no be null.");
$assert(x,"node can no be null.");
$assert("order can no be null.");
var y=this._getSortedChildren(r,x);
var q=y.map(function(a){return{id:a.getId(),height:this._computeChildrenHeight(r,a)}
}.bind(this));
var i={};
for(var t=0;
t<q.length;
...
var d=r.x;
var e=r.y;
var f={x:d+b.x,y:e+b.y+this._calculateAlignOffset(w,a,t)};
this._treeSet.updateBranchPosition(a,f)
}.bind(this));
w._branchHeight=v
}y.each(function(a){this._layoutChildren(a,t)
},this)
...
}e.each(function(a){this._fixOverlapping(a,d)
},this)
},_shiftBranches:function(i,k){var h=[i];
var g=this._treeSet.getSiblingsInVerticalDirection(i,i.getFreeDisplacement().y);
var j=i;
g.each(function(b){var c=h.some(function(d){return this._branchesOverlap(d,b,k)
},this);
...
l.each(function(a){var b=i.getFreeDisplacement().y;
